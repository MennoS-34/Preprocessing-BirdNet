...
logging.basicConfig(filename='Test_evaluation_Version 3.log', level=logging.INFO,
...
xy_test_normalized = np.load(os.path.join(DATA_FOLDER, "test_normalized.npy"))
...
def separating_spectrograms(dataset, block_height, block_width):
    ...
    logger.info("Spectogram adding to array.")
    block_dataset = np.array(block_dataset)
    logger.info("Spectogram split into blocks successfully.")
...
def predicting_full(block_dataset_normalized, model, spectogram_h, spectogram_w, block_height, block_width, block_depth):
    ...
    N_spectogram = block_predicting.shape[0] // N_blocks

    block_predicting = block_predicting.reshape(N_spectogram, N_blocks, block_height, block_width, block_depth)
