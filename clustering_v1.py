...
def save_selected_filenames(selected_filenames, ANNOTATIONS_PATH):
    ...
    for cluster, filenames in selected_filenames.items():
        ...
        logging.info(f"Cluster {cluster+1}: Selected {len(filenames)} filenames for manual annotation")
        logging.info(f"Annotations saved to {annotation_file}")

# Main execution
if __name__ == "__main__":
    # Load the predictions_latent_space
    predictions_latent_space = np.load('/home/u993985/Thesis 2/Results/predictions - Version 1.npy')  # Adjust the path accordingly
