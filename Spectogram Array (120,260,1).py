...
def process_image(file_path: str) -> np.ndarray:
    """
    Load an image, resize it to 120x160, convert it to grayscale, and return it as a numpy array.
...
def convert_images_to_numpy_array(image_dir: str, output_file: str, batch_size: int, max_workers: int = 4) -> None:
    ...
    """
    print(f"The following spectogram directory is used: {image_dir}")
    ...
    file_paths = [os.path.join(image_dir, filename) for filename in os.listdir(image_dir) if filename.endswith('.png')]
...
# Example usage
image_directory = "D:/Menno/Thesis/Expirement/Spectograms/Train/"
output_file_path = "D:/Menno/Thesis/Expirement/Spectograms/train.npy"

batch_size = 250000  # Adjust based on available memory and number of images
